...
macOSの「ミュージック」アプリをプレイヤーとして使う、ローカル動作のイントロクイズです。宴会や友人同士の集まりで、曲のイントロを聴いてタイトルを当てて遊べます。
...
Music.appに `イントロドン` または `Intro Don` という名前のプレイリストを作り、出題したい曲を入れておくとスムーズです。

- `イントロドン` または `Intro Don` というプレイリストがあれば自動選択します。
...
初回はブラウザの言語設定に合わせて表示され、画面上部の言語メニューから日本語と英語を切り替えられます。
